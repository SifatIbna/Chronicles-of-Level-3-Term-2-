...
int expectedPktNumA = 0;
int expectedPktNumB = 1;
...
//    int i;
    numberPktA++;
    printf("\nReceived packet %d [%s] at time %f\n",numberPktA,message.data,time);
...
void A_input(struct pkt packet)
{
    ...
    if ((chksum != packet.checksum) || (packet.seqnum != expectedPktNumA))
    {
        printf("Packet: %d\n",packet.seqnum);
        printf("Packet Ack Num. %d\n",packet.acknum);
        printf("Packet Checksum :%d\n",packet.checksum);
        printf("Found Checksum: %d\n",chksum);
...
void A_init(void)
{
    printf("Initializing the packet to A\n");
...
void B_input(struct pkt packet)
{
    ...
    if (packet.checksum != chksum){
        ...
        if (prevPacketNoB != packet.seqnum){
            tolayer5(1,packet.payload);
            prevPacketNoB = packet.seqnum;
        }
